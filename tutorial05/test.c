...
static void test_parse_array() {
    ...
    /* cannot use a single macro to check test, use API to check seperately */
    lept_init(&v);
    ...
    for (i = 0; i < lept_get_array_size(&v); i++) {
        lept_value *inside_v = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(inside_v));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(inside_v));
        for (j = 0; j < i; j++) {
            lept_value *ele = lept_get_array_element(inside_v, j);
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(ele));
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(ele));
        }
    }
    lept_free(&v);
}
